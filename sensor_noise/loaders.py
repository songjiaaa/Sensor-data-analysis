...
def _find_time_column(columns: list[str]) -> str | None:
    for col in columns:
        ...
        if "时间" in str(col) or normalized in {
            "time",
            "t",
            "timestamp",
            "time_s",
...
def _detect_channel_columns(columns: list[str], time_col: str | None = None) -> list[str]:
    ...
    fallback = [
        col
        for col in columns
        if _normalize_col(col) not in excluded
    ]
    return fallback
...
def _parse_meta_rows(raw_rows: list[list[str]]) -> RecordingMeta:
    ...
    for row in raw_rows:
        ...
        if len(cells) == 1 and "," in cells[0]:
            ...
            if len(parts) >= 2 and parts[0] in META_KEYS:
...
def _row_looks_like_header(row: pd.Series) -> bool:
    ...
    if not values:
        return False
    has_time = _find_time_column(values) is not None
    has_channel = len(_detect_channel_columns(values, _find_time_column(values))) > 0
...
def _infer_sample_rate(meta: RecordingMeta, time_s: np.ndarray) -> float:
    if meta.avg_sample_rate_hz > 0:
        return meta.avg_sample_rate_hz
    if meta.nominal_sample_rate_hz > 0:
        return meta.nominal_sample_rate_hz
    if len(time_s) > 1:
        dt = np.diff(time_s)
        positive_dt = dt[dt > 0]
...
def _finalize_recording(path: Path, meta: RecordingMeta, df: pd.DataFrame) -> Recording:
    ...
    df = df[[time_col, *channel_cols]].copy()
    for col in [time_col, *channel_cols]:
    ...
    sample_rate = _infer_sample_rate(meta, time_s)
    if sample_rate <= 0:
        raise ValueError(
...
def _load_xlsx(path: Path) -> Recording:
    preview = pd.read_excel(path, header=None, nrows=120)
    if preview.empty:
        raise ValueError("Excel 文件为空")
    ...
    meta = _parse_meta_rows([[str(v) for v in preview.iloc[i].tolist()] for i in range(header_idx)])
    raw = pd.read_excel(path, header=None)
    df = _build_dataframe_from_raw(raw, header_idx)
